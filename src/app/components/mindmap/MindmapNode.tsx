  ...
  return createPortal(
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="mindmap-leaf-modal-title"
        className="w-[600px] max-w-[90vw] rounded-xl border border-white/10 bg-[#0f1923] shadow-2xl"
      >
        <div className="flex items-center justify-between border-b border-white/5 px-4 py-3">
          <span
            id="mindmap-leaf-modal-title"
...
export function MindmapNode({ id, data }: NodeProps & { data: MindmapNodeData }) {
    ...
    return (
      <>
        <div
          ...
          className="w-[320px] cursor-pointer rounded-lg bg-[#16202c] border border-[#2a3b4d] shadow-lg hover:border-white/25 transition-colors"
  ...
  return (
    <div
      className="min-w-[150px] max-w-[250px] rounded-lg bg-[#16202c] border border-[#2a3b4d] shadow-lg"
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        <span
          ...
          aria-hidden="true"
          className={`h-2 w-2 flex-shrink-0 rounded-full ${levelStyle}`}
        />
        {editing ? (
          <textarea
            ref={textareaRef}
          ...
          <p className="min-w-0 flex-1 text-sm text-white truncate" title={data.content}>
            {data.content}
          </p>
        )}
      </div>
