...
export function QuickAccessSkillsEditor({
  ...
  const addSkill = () => {
    ...
    onAnnounce(`Added skill ${skills.length + 1}`);
  };
  ...
  const move = (index: number, offset: -1 | 1) => {
    ...
    onAnnounce(`Moved ${next[target].label || 'skill'} to position ${target + 1}`);
  };
  ...
  const remove = (index: number) => {
    ...
    onAnnounce(`Removed ${removed.label || 'skill'}`);
  };
  ...
  return (
    <SettingsSection title="Skills" icon="auto_awesome">
      <ul aria-label="Skills" className="flex flex-col gap-2">
        {skills.map((skill, index) => {
          const provider = providers.find((p) => p.id === skill.providerId);
          const name = skill.label || `skill ${index + 1}`;
          return (
            <li
              key={skill.id}
            ...
            >
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder="Blogartikel"
                  onChange={(event) => update(index, { label: event.target.value })}
                  className={INPUT_CLASS}
                />
                <input
                  value={skill.prompt}
                  aria-label={`Skill ${index + 1} prompt`}
                  placeholder="/blogartikel"
              ...
              <details className="mt-2">
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  <select
                    aria-label={`Skill ${index + 1} provider`}
                  ...
                  <select
                    aria-label={`Skill ${index + 1} model`}
                    value={skill.model ?? ''}
                    disabled={!provider}
                  ...
                  <select
                    aria-label={`Skill ${index + 1} permission mode`}
      ...
      <button
        type="button"
        data-testid="quick-access-add-skill"
        onClick={addSkill}
