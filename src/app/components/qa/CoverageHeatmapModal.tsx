            ...
            {hovered && (
                <Html position={[0, height / 2 + 1, 0]} center zIndexRange={[100, 0]}>
                    <div className="bg-black/90 text-white px-3 py-2 rounded-lg text-xs border border-white/20 pointer-events-none shadow-xl min-w-[150px]">
                        <strong className="block mb-1 text-primary-light truncate max-w-[200px]" title={file.path}>
                            {file.path.split('/').pop()}
                        </strong>
                        <div className="text-white/70 text-[10px] mb-2 truncate max-w-[200px]" title={file.path}>
                            {file.path}
                        </div>

                        <div className="grid grid-cols-2 gap-x-3 gap-y-1">
                            <span className="text-white/50">Coverage:</span>
                            <span className={`font-bold tabular-nums text-right ${coverage >= 80 ? 'text-emerald-400' : coverage >= 50 ? 'text-amber-400' : 'text-rose-400'}`}>
                                {Math.round(coverage)}%
                            </span>

                            <span className="text-white/50">Size/Cmplx:</span>
                            <span className="font-bold tabular-nums text-right text-white">
                                {file.size}
                            </span>
                            ...
                            {risk > 0.2 && (
                                <>
                                    ...
                                    <span className="font-bold tabular-nums text-right text-rose-500">
                                        High
                                    </span>
                                </>
                            )}
            ...
            >
                ...
                <div className="flex items-center justify-between border-b border-white/8 bg-white/2 px-6 py-4">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-sm font-bold tracking-wide text-foreground">3D Coverage Code City</h2>
                ...
                <div className="flex-1 relative bg-black/30">
                    ...
                    <div className="absolute top-4 left-4 pointer-events-none flex items-center gap-2 bg-black/40 px-3 py-1.5 rounded-full border border-white/5 backdrop-blur-sm">
                        ...
                        <span className="text-[10px] font-medium text-white/60">Drag to rotate • Scroll to zoom</span>
                    </div>
                    ...
                    <div className="absolute bottom-4 left-4 bg-[#0e0e14]/90 border border-white/10 p-4 rounded-xl backdrop-blur-md pointer-events-none min-w-[180px]">
                        <h3 className="text-[10px] font-bold uppercase tracking-widest text-foreground-muted mb-3">Legend</h3>
                        ...
                        <div className="border-t border-white/10 pt-3 space-y-2">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="material-symbols-outlined text-[14px] text-rose-500">local_fire_department</span>
